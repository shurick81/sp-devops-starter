...
Start-Process -FilePath "java" -ArgumentList "-jar swarm-client-3.9.jar -name $env:computername -disableSslVerification -master http://$env:SPDEVOPSSTARTER_JENKINSHOST`:16080 -username admin -password admin -labels `'sharepoint-auto-$($env:SPDEVOPSSTARTER_JENKINSLABEL)`' -executors 2";
